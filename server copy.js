...
async function extractProducts(page) {
    ...
    console.log('Starting Hybrid "Grid Scrape + Click for URL" extraction...');

    // The snippet shows data-testid="item-collection-card-full" is INSIDE the wrapper,
    // and the price is a SIBLING in the wrapper.
    const CARD_SELECTOR = 'div[data-testid="item-collection-card-full"]';

    try {
        await page.waitForSelector(CARD_SELECTOR, { timeout: 10000 });
    } catch (e) {
        console.log('Timeout waiting for card items.');
        return [];
    }

    const cardsCount = await page.locator(CARD_SELECTOR).count();
    console.log(`Found ${cardsCount} items. Limiting manual scrape to first 26.`);

    const products = [];
    const loopLimit = Math.min(cardsCount, 26);

    for (let i = 0; i < loopLimit; i++) {
        try {
            console.log(`Processing item ${i + 1}/${cardsCount}...`);

            const card = page.locator(CARD_SELECTOR).nth(i);

            // 1. Scrape Details from the GRID CARD (Reliable)
            // Using selectors confirmed from user HTML snippet
            // CRITICAL CHANGE: We use the PARENT element to find the SIBLING price/weight info.
            const gridData = await card.evaluate((node) => {
                // Determine the root for scraping (the wrapper div)
                const root = node.parentElement || node;
                const text = root.innerText; // Capture full text including siblings

                const nameEl = root.querySelector('div.iPErou') || root.querySelector('div[data-testid="item-name"]');
                const name = nameEl ? nameEl.innerText.trim() : '';

                const imgEl = root.querySelector('img._16I1D') || root.querySelector('img');
                const image = imgEl ? (imgEl.getAttribute('src') || imgEl.getAttribute('data-src')) : '';

                // Weight Extraction
                let weight = '';
                // Updated selector based on user snippet: ._30iun > .bCqPoH
                const weightEl = root.querySelector('div._30iun div.bCqPoH') || root.querySelector('div.bCqPoH');
                if (weightEl) {
                    weight = weightEl.innerText.trim();
                } else {
                    // Fallback: Regex for weight (e.g., "500 g", "1 kg", "200 ml")
                    const weightMatch = text.match(/(\d+(\.\d+)?\s*(g|kg|ml|l|ltr|pcs|pack|units))\b/i);
                    if (weightMatch) weight = weightMatch[0];
                }

                // Description
                const descEl = root.querySelector('div.diZRny');
                const description = descEl ? descEl.innerText.trim() : '';

                // Prices
                // Selectors from snippet: Current=.iQcBUp, Original=.dTWhKm
                const priceEl = root.querySelector('div.iQcBUp');
                const mrpEl = root.querySelector('div.dTWhKm');

                let currentPrice = priceEl ? parseFloat(priceEl.innerText.replace(/[^\d.]/g, '')) : 0;
                let originalPrice = mrpEl ? parseFloat(mrpEl.innerText.replace(/[^\d.]/g, '')) : 0;

                // Fallback Price Logic (Regex on full text)
                if (!currentPrice) {
                    // Match ₹90 or just 90 if it looks like a price (start of line or after space?)
                    // Safer to rely on ₹ first.
                    let priceMatches = text.match(/₹\s*(\d+(\.\d+)?)/g);
                    if (priceMatches) {
                        const p1 = parseFloat(priceMatches[0].replace(/[^\d.]/g, ''));
                        currentPrice = p1;
                        if (priceMatches.length > 1) {
                            const p2 = parseFloat(priceMatches[1].replace(/[^\d.]/g, ''));
                            if (p2 > p1) originalPrice = p2;
                        }
                    }
                    // No dangerous fallback to ANY number without context, too risky.
                }

                if (!originalPrice && currentPrice) originalPrice = currentPrice;
                if (!currentPrice && originalPrice) currentPrice = originalPrice;

                const discountEl = root.querySelector('div[data-testid="offer-text"]');
                let discountPercentage = 0;
                if (discountEl) {
                    const match = discountEl.innerText.match(/(\d+)%/);
                    if (match) discountPercentage = parseInt(match[1]);
                } else if (originalPrice > currentPrice) {
                    discountPercentage = Math.round(((originalPrice - currentPrice) / originalPrice) * 100);
                }

                const addButton = root.querySelector('div[data-testid="buttonpair-add"]');
                const isOutOfStock = !addButton;

                const soldOutText = text.toLowerCase().includes('sold out');

                const badgewrapper = root.querySelector('div[data-testid="badge-wrapper"]');
                const isAd = badgewrapper && badgewrapper.innerText.toLowerCase().includes('ad');
                ...
                return {
                    ...
                    originalPrice,
                    discountPercentage,
                    isOutOfStock: isOutOfStock || soldOutText,
                    isAd: !!isAd,
                    deliveryTime: null,
                    debugText: (currentPrice === 0) ? text : null
                };
            });

            if (gridData.debugText) {
                console.log(`[DEBUG] Price is 0 for "${gridData.productName}". Card Text (Parent Scoped):\n${gridData.debugText}`);
            }

            // 2. CLICK to open Popup (Solely to get URL)
            await card.click({ force: true, delay: 100 });

            try {
                const popupSelector = '#product-details-page-container, div._1ne2g';
                await page.waitForSelector(popupSelector, { timeout: 2000 });
            } catch (e) { }

            // 3. Capture Product URL
            const productUrl = page.url();

            // 4. Close Popup / Go Back
            const backBtn = page.locator('button[data-testid="simpleheader-back"]');
            if (await backBtn.isVisible()) {
                await backBtn.click();
                await page.waitForSelector(CARD_SELECTOR, { timeout: 2000 }).catch(() => { });
            } else {
            ...
            // 5. Combine & Push
            if (gridData.productName) {
                products.push({
                    ...gridData,
                    productUrl: productUrl, // The captured URL
                    platform: "instamart",
                    ranking: i + 1,
            ...
            console.log(`Error processing item ${i}: ${err.message}`);
        }
    }
    ...
    try {
        ...
        page.on('response', async response => {
            ...
            if (resUrl.includes('api/instamart/') && (resourceType === 'fetch' || resourceType === 'xhr')) {
                ...
                if (resUrl.includes('category/list') || resUrl.includes('listing')) {
                    // console.log(`Intercepted Possible Category API: ${resUrl}`);
                    try {
                        ...
                        if (status >= 200 && status < 300) {
                            ...
                            const parsed = processCapturedJson(json);
                            if (parsed.length > 0) {
                                console.log(`API gave ${parsed.length} products.`);
                                parsed.forEach(p => {
                            ...
                            // Save dump just in case
                            const safeName = resUrl.replace(/[^a-zA-Z0-9]/g, '_').substring(0, 50);
                            const filename = path.join(__dirname, `api_success_${safeName}_${Date.now()}.json`);
        ...
        // User requested FULL scroll to dump all API data, but strictly limit DOM interaction later.
